package com.ezzenix.chatanimation.lib.config;
...
public class ConfigScreen extends Screen {
	...
	public void updateList() {
		...
		for (EntryInfo info : BaseConfig.entries) {
			...
			//? if >1.20.1 {
			SpriteIconButton resetButton = SpriteIconButton.builder(Component.literal("Reset"),
			//?} else {
			/*TextAndImageButton resetButton = TextAndImageButton.builder(Component.literal("Reset"), new Identifier(BaseConfig.getModId(), "textures/gui/sprites/reset.png"),
			 *///?}
				(b -> {
			...
			//? if >1.20.1 {
				// use tryParse() because its the only method that doesn't change between versions
				, true).sprite(Objects.requireNonNull(Identifier.tryParse(BaseConfig.getModId()+":reset")), 12, 12).size(20, 20).build();
			//?} else {
			...
			if (info.getType() == boolean.class) {
				...
				/* enum cycle */
				Button button = Button.builder(Component.literal(info.getValue().toString()), (b) -> {
					Object[] constants = info.getType().getEnumConstants();
					Enum<?> currentValue = (Enum<?>) info.getValue();

					int nextIndex = (currentValue.ordinal() + 1) % constants.length;
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		//? if <=1.20.1
		//super.extractRenderStateBackground(graphics);
